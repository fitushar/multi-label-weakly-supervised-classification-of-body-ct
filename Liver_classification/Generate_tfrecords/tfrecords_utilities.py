...
from Preprocessing_utlities import extract_Liver_center_patch
from Preprocessing_utlities import Get_Center_from_a_binary_mask
from Preprocessing_utlities import extract_Liver_center_patch_updated
...
def Save_pngAndmask(a_id,a_lbl,img1,mask1,path):
    ...
    decoding_lbl=[]
    if p_lbl[0]==1:
        decoding_lbl.append('Liver Stone')
    if p_lbl[1]==1:
        decoding_lbl.append('Liver Lesion')
    if p_lbl[2]==1:
        decoding_lbl.append('Dilatation')
    if p_lbl[3]==1:
        decoding_lbl.append('Fatty Liver')
    if p_lbl[4]==1:
        decoding_lbl.append('Liver Normal')
    print(decoding_lbl)
    ...
    last_plot = axarr[0][2].imshow(np.squeeze(img1[95, :, :]), cmap='gray',origin='lower');
    axarr[0][2].axis('off')
    axarr[0][2].set_title('Last-Slice',fontsize = 128)
    ...
    mask_last_plot = axarr[1][2].imshow(np.squeeze(img1[95, :, :]), cmap='gray',alpha=ALPHA_VALUES_CT,origin='lower');
    mask_last_plot = axarr[1][2].imshow(np.squeeze(mask1[95, :, :]), cmap=COLORMAP,alpha=ALPHA_VALUES_MASK,origin='lower');
    axarr[1][2].axis('off')
...
def creat_tfrecord(df,extraction_perameter,tf_name,path):
    ...
    ct_patch,mask_patch=extract_Liver_center_patch_updated(image ,mask,subject_id,path)
