...
def extract_Liver_center_patch(i,l,NameforCenterSlicePNG,pathpng):
    ...
    try:
        ...
        for i in range(mask.shape[0]):
            roi_slice=mask[i, :, :]
            slice_flatten=roi_slice.flatten()
            Liver_idx=np.argwhere(slice_flatten == 5)
            Liver_slice_Dice.append(len(Liver_idx))
            Liver_slice_number.append(i)
            if (len(Liver_idx)!=0):
                Liver_slice_count.append(1)
            else:
                Liver_slice_count.append(0)
        #--Making dataframe
        Liver_dice_And_slice=pd.DataFrame(list(zip(Liver_slice_number,Liver_slice_Dice,Liver_slice_count)),columns=['s_n','s_d','Livr_slice_Count'])
        Liver_dice_And_slice
        max_dice=np.max(Liver_slice_Dice)
        where_is_max_dice=np.argwhere(Liver_slice_Dice==max_dice)
        ...
        ####----
        only_Liver_slices=Liver_dice_And_slice[(Liver_dice_And_slice['Livr_slice_Count']==1) & (Liver_dice_And_slice['s_d']>=100) ]

        liver_first_slice=(only_Liver_slices['s_n'].iloc[0])
        liver_last_slice=(only_Liver_slices['s_n'].iloc[-1])

           #-------------------------------------------------------------#
           #---------- taking the center slice to get the Liver Center---#
           #-------------------------------------------------------------#
        mask_a=mask[Center_slice, :, :]
        #mask_a=mask_ai[0,:,:]
        print(mask_a.shape)
        Liver_mask= ((mask_a == 5)).astype(np.int32)
        body_mask = ((mask_a != 0)).astype(np.int32)
        ##-thresholding mask
        thresh = threshold_mean(Liver_mask)
        binary = Liver_mask > thresh

        ##-- to remove over segmented region
        binary=ndimage.binary_erosion(binary,structure=np.ones((11,11))).astype(int)
        ##-- Covering holes
        ...
        ###---Getting Center of the binary Liver Mask
        cx,cy=Get_Center_from_a_binary_mask(Png_name)
        if (Center_slice < 128):
            slicer = [slice(liver_first_slice,  liver_last_slice),slice(int(cy) - 64, int(cy) +64),slice(cx - 64, cx +64)]
            ex_image = img[slicer]
            ex_lbl = mask[slicer]
            ex_image=resize_image_with_crop_or_pad(ex_image, [256,128,128], mode='constant')
            ex_lbl=resize_image_with_crop_or_pad(ex_lbl, [256,128,128], mode='constant')
            print('Using method-alternative')
        else:
            slicer = [slice(Center_slice-128,  Center_slice+128),slice(int(cy) - 64, int(cy) +64),slice(int(cx) - 64, int(cx)+64)]
            ex_image = img[slicer]
            ex_lbl = mask[slicer]
            ex_image=resize_image_with_crop_or_pad(ex_image, [256,128,128], mode='constant')
            ex_lbl=resize_image_with_crop_or_pad(ex_lbl, [256,128,128], mode='constant')
            print('Using --Liver--Center--Method')
    except:
       ...
       #####----z dimention-----######
        ...
        image=resize_image_with_crop_or_pad(image, [dimention1,dimention2,dimention3], mode='constant')
        mask=resize_image_with_crop_or_pad(mask, [dimention1,dimention2,dimention3], mode='constant')
        print('######before padding image shape--{}#####'.format(image.shape))
        ...
        images,masks = extract_class_balanced_example_array(
                    image,mask,
                    example_size=[256,128,128],
                    n_examples=1,
                    classes=6,class_weights=[0,0,0,0,0,1])
...
def extract_Liver_center_patch_updated(i,l,NameforCenterSlicePNG,pathpng):

    ###Getting the label
    mask=l
    img=i
    PatchDepth=96
    PatchDepthDevideBy2=(PatchDepth/2)
    try:
        ##--Making Description list
        Liver_slice_number=[]
        Liver_slice_Dice=[]
        ...
        ##Fixing x-boundary
            ...
            slicer = [slice(liver_first_slice,  liver_last_slice),slice(center_y_lr, center_y_upr),slice(center_x_lr, center_x_upr)]
            ex_image = img[slicer]
            ex_lbl = mask[slicer]
            ex_image=resize_image_with_crop_or_pad(ex_image, [PatchDepth,128,128], mode='constant')
            ex_lbl=resize_image_with_crop_or_pad(ex_lbl, [PatchDepth,128,128], mode='constant')
            print('----Using method-alternative----')
       ...
       #####----z dimention-----######
        ...
        image=resize_image_with_crop_or_pad(img, [dimention1,dimention2,dimention3], mode='constant')
        mask=resize_image_with_crop_or_pad(mask, [dimention1,dimention2,dimention3], mode='constant')
        print('######before padding image shape--{}#####'.format(image.shape))
        ...
        images,masks = extract_class_balanced_example_array(
                    image,mask,
                    example_size=[PatchDepth,128,128],
                    n_examples=1,
                    classes=6,class_weights=[0,0,0,0,0,1])
