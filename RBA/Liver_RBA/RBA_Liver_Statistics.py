...
organ_dictonary=PATH_TO_SAVE_CSV+"liver_dict.pickle"
Diagnosis_list = LIST_FOR_OVERLAP_STATISTICS
Disease_Name_and_Number_Decending_CSV_NAME=PATH_TO_SAVE_CSV+"Liver_Disease_Name_and_Number_Decending.csv"
...
for dis in Greater_than_200_disease:
    if dis=='surgically\s+absent':
        name= PATH_TO_SAVE_CSV+'surgically_absent.csv'
        cases = liver_dict[dis]
        Writting_disease_CSV(lst=cases,main_lst=report_list,Name=name)
    elif dis=='fatty\\s+infiltr':
        name= PATH_TO_SAVE_CSV+'fatty_infiltr.csv'
        cases = liver_dict[dis]
        Writting_disease_CSV(lst=cases,main_lst=report_list,Name=name)
    else:
        name=PATH_TO_SAVE_CSV+dis+'.csv'
        cases = liver_dict[dis]
        Writting_disease_CSV(lst=cases,main_lst=report_list,Name=name)
...
for i in range (0, len(Greater_than_200_disease)):
    ...
    for j in range(0,len(Greater_than_200_disease)):
        ...
        first_csv=PATH_TO_SAVE_CSV+Greater_than_200_disease[i]+'.csv'
        Second_csv=PATH_TO_SAVE_CSV+Greater_than_200_disease[j]+'.csv'
...
df_cm.to_csv(PATH_TO_SAVE_CSV+"LiverDiagnosus0verlap.csv", encoding='utf-8', index=False)

plt.figure(figsize = (40,40))
sn.heatmap(df_cm, annot=True,fmt="d")
plt.savefig(PATH_TO_SAVE_CSV+'LiverDiagnosus0verlap.png')
