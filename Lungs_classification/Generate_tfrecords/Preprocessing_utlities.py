...
def resize_image_with_crop_or_pad(image, img_size=(112, 112, 112), **kwargs):
...
def extract_random_example_array(image_list,example_size=[1, 112, 112],n_examples=1):
...
def extract_Lung_center_patch_updated_problemsSolvedCenter(i,l,NameforCenterSlicePNG,pathpng):
    ...
    try:
        ...
        for i in range(mask.shape[0]):
            roi_slice=mask[i, :, :]
            K= ((roi_slice == 2)|(roi_slice == 3)).astype(np.int32)
            slice_flatten=K.flatten()
            Lung_idx=np.argwhere(slice_flatten==1)
            Lung_slice_Dice.append(len(Lung_idx))
            Lung_slice_number.append(i)
            if (len(Lung_idx)!=0):
                Lung_slice_count.append(1)
            else:
                Lung_slice_count.append(0)
        #--Making dataframe
        Lung_dice_And_slice=pd.DataFrame(list(zip(Lung_slice_number,Lung_slice_Dice,Lung_slice_count)),columns=['s_n','s_d','Livr_slice_Count'])
        Lung_dice_And_slice
        max_dice=np.max(Lung_slice_Dice)
        where_is_max_dice=np.argwhere(Lung_slice_Dice==max_dice)
        ...
        ####----
        only_Lung_slices=Lung_dice_And_slice[(Lung_dice_And_slice['Livr_slice_Count']==1) & (Lung_dice_And_slice['s_d']>=100) ]

        Lung_first_slice=(only_Lung_slices['s_n'].iloc[0])
        Lung_last_slice=(only_Lung_slices['s_n'].iloc[-1])

           #-------------------------------------------------------------#
           #---------- taking the center slice to get the Lung Center---#
           #-------------------------------------------------------------#
        mask_a=mask[Center_slice, :, :]
        #mask_a=mask_ai[0,:,:]
        print('mask_shape')
        print(mask_a.shape)
        Lung_mask= ((mask_a == 2)|(mask_a == 3)).astype(np.int32)
        body_mask = ((mask_a != 0)).astype(np.int32)
        ##-thresholding mask
        thresh = threshold_mean(Lung_mask)
        binary = Lung_mask > thresh
        ...
        ##Fixing x-boundary
            ...
            x_lower_bound = int(cx) - 80
            if (x_lower_bound <0):
                center_x_lr=0
            else:
                center_x_lr=x_lower_bound
            # Uper boundary of x-center
            x_uper_bound  = int(cx) + 80
            if (x_uper_bound >img.shape[2]):
                center_x_upr=img.shape[2]
            else:
                center_x_upr=x_uper_bound

            slicer = [slice(Lung_first_slice,  Lung_last_slice),slice(center_y_lr, center_y_upr),slice(center_x_lr, center_x_upr)]
            ex_image = img[slicer]
            ex_lbl = mask[slicer]
            ex_image=resize_image_with_crop_or_pad(ex_image, [224,160,160], mode='constant',constant_values=0)
            ex_lbl=resize_image_with_crop_or_pad(ex_lbl, [224,160,160], mode='constant',constant_values=0)
            ...
            if (x_uper_bound >img.shape[2]):
                ...
                center_x_upr=x_uper_bound
            slicer = [slice(Center_slice-112,  Center_slice+112),slice(center_y_lr, center_y_upr),slice(center_x_lr, center_x_upr)]
            ex_image = img[slicer]
            ex_lbl = mask[slicer]
            ex_image=resize_image_with_crop_or_pad(ex_image, [224,160,160], mode='constant',constant_values=0)
            ex_lbl=resize_image_with_crop_or_pad(ex_lbl, [224,160,160], mode='constant',constant_values=0)
            print('---Using --Lung--Center--Method---')
    except:

        mask=((mask == 2)|(mask == 3)).astype(np.int32)
        print("Going Class balanced Policy")
        patch_size=[224,160,160]
        img_shape=img.shape
